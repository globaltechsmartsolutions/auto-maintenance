...
| 9 | AI governance setup | **Delivered in code:** per-company feature flag, authorised monthly token budget, per-feature hourly rate limit, a global and a per-company kill switch, an `ai.<feature>.<outcome>` audit convention with usage records for refused calls too, output safety checks, and the five-scenario evaluation set. See `docs/WIACONTROL_AI_GOVERNANCE.md`. **Owner tasks:** provider and processing location, DPA, subprocessor terms, privacy sign-off, and a named cost owner. | 2; privacy owner approval | AI remains disabled for all customers until these controls and test cases are approved. |
...
and off by default at four independent levels: the environment flag, the
per-company feature list, the kill switches, and the authorised monthly token
budget. The incident draft can now reach a recipient, but only through explicit
human approval that records the approver and the exact text they accepted. The
full control set is documented in `docs/WIACONTROL_AI_GOVERNANCE.md`.
...
The developer must complete the dedicated [Staging QA Checklist](WIACONTROL_STAGING_QA_CHECKLIST.md)
before the controlled pilot. It contains the fictional company setup, 73
checkable cases, evidence requirements, and the release decision.
