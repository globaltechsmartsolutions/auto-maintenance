...
| Communications | Implemented foundation | A scheduled outbox worker delivers in-app messages, retries failures with bounded backoff, recovers expired processing leases, and supports recipient acknowledgement and coordinator resend. | Configure the email provider and verify delivery/recovery flows in staging before enabling external email. |
